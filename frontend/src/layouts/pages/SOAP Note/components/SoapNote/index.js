...
import { getDoctorSoapFormListAction } from "store/actions/actions";
...
function SoapNote() {
    ...
    const soapNotesdata = useSelector((state) =>
        state.auth.doctorSoapNotes.result !== undefined ? state.auth.doctorSoapNotes.result : []
    );

    const totalCount = useSelector((state) => state.auth.doctorSoapNotes.totalCount);
