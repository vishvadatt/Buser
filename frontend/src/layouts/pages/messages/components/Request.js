...
const Request = () => {
  ...
  const getChats = () => {
    const unsub = onSnapshot(doc(db, "messageRequest", loginUser._id), (doc) => {
      setChats(doc.data());
      console.log("vvx",doc.data())
    }); 
    return () => {
      unsub();
    };
  ...
  const acceptRequest = async (item) => {
    ...
    try {
      ...
      if (!res.exists()) {
        ...
        await updateDoc(doc(db, "userChats", item[1].userInfo.uid), {
          [combinedId + ".date"]: serverTimestamp(),
          [combinedId + ".unreadCount"]: increment(1),
          [combinedId + ".isFriend"]: true,
        });

        const chatRef = doc(db, "messageRequest", loginUser?._id);
      await updateDoc(chatRef, {
        [combinedId]: deleteField(),
      });
      }
      const res1 = await getDoc(doc(db, "videoChats", combinedId));
      console.log("res.exists()", res1.exists());
      if (!res.exists()) {
        //create a chat in chats collection
        ...
        if (docSnap.exists()) {
          // document already exists, update it
          await updateDoc(doc(db, "userVideoChats", loginUser?._id), {
            [combinedId + ".userInfo"]: {
              uid: item[1].requestSenderID,
              displayName: item[1].userInfo.displayName,
              photoURL: item[1].userInfo.photoURL ? item[1].userInfo.photoURL : "",
        ...
        await updateDoc(doc(db, "userVideoChats", item[1].userInfo.uid), {
          [combinedId + ".date"]: serverTimestamp(),
          [combinedId + ".unreadCount"]: increment(1),
          [combinedId + ".isFriend"]: true,
        });
  ...
  const rejectRequest = async (item) => {
    ...
    try {
      const chatRef = doc(db, "userChats", item[1].userInfo.uid);
      await updateDoc(chatRef, {
        [combinedId]: deleteField(),
      });

      const chatRef2 = doc(db, "messageRequest", loginUser?._id);
  ...
  return (
    ...
    <div className="chatlist">
      {chats && Object.entries(chats)
        ...
        {
          chats && Object.entries(chats).length === 0 &&  <div style={{display:"flex", justifyContent:"center"}}>No Request Found</div>
        }
    </div>
    </>
  );
